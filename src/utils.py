...
def get_data_directory(data_embedded_column_name: str, random_seed: int) -> str:
    return os.path.join(
        os.path.dirname(__file__), os.pardir, "data", f"seed_{random_seed}", data_embedded_column_name
    )


def get_model_save_directory(data_embedded_column_name: str, embedding_model_name: str,
                             target_column_name: str, bag_size: int, baseline: str,
                             random_seed, dev, task_type, prefix, multiple_runs=False) -> str:
    model_name = baseline
    folder_name = "runs"
    if multiple_runs:
