...
from models.mil import MaxMLP, MeanMLP, AttentionMLP, ApproxRepSet, random_model, majority_model
from utils import (
    AverageMeter,
    EarlyStopping,
...
if __name__ == "__main__":
    ...
    run_dir = get_model_save_directory(data_embedded_column_name=args.data_embedded_column_name,
                                       ...
                                       bag_size=args.bag_size, baseline=args.baseline, random_seed=args.random_seed, 
    ...
    if args.task_type == 'classification':
        if args.baseline == "random":
            random_model(train_dataframe, test_dataframe, args, logger)
            exit()

        if args.baseline == 'majority':
            majority_model(train_dataframe, test_dataframe, args, logger)
            exit()
    ...
    if args.run_sweep:
        ...
        args.sweep_config["name"] = f"{args.label}_{args.baseline}"
