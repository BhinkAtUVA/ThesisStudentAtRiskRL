...
from models import DebiasingPolicyNetwork, PolicyNetwork, sample_action, select_from_action, create_mil_model_with_dict
from utils import (
    get_data_directory,
...
def finish_episode_policy_only(
    ...
    for batch_x, batch_y, _, _  in train_dataloader:
        ...
        sel_y = batch_y
        sel_loss, bias_loss = policy_network.train_minibatch(sel_x, sel_y)
    ...
    optimizer.step()
    policy_network.task_optim.step()
...
def finish_episode(
    ...
    sel_losses, regularization_losses, bias_losses = [], [], []
    for batch_x, batch_y, indices, _ in train_dataloader:
        policy_network.train()
    ...
    regularization_loss = torch.stack(regularization_losses).mean() / 100
    bias_loss = torch.stack(bias_losses).mean()
    total_loss = policy_loss + value_loss + reg_coef * regularization_loss - 1.0 * bias_loss
...
def create_rl_model(args, mil_best_model_dir):
    ...
    task_model = load_mil_model_from_config(os.path.join(mil_best_model_dir, "..", "best_model_config.json"),
                                            state_dict)
    policy_network = DebiasingPolicyNetwork(task_model=task_model,
                                            state_dim=args.state_dim,
                                            hdim=args.hdim,
                                            hidden_dim=32, #TODO: Add real hidden dim
